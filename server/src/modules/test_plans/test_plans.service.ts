...
export async function GetTestPlans(releaseId: number) {
  const result = await pool.query(
    `SELECT tp.id, tp.name, tp.created_by, tp.created_at
     FROM test_plans tp
...
export async function cloneTestPlan(planId: number) {
  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    ...
    // await client.query(
    //   `INSERT INTO release_test_plans (plan_id)
    //    VALUES ($1, $2)`,
    //   [newPlan.id],
    // );
...
export async function GetReleasesTestPlansByProject(projectId: number) {
  const result = await pool.query(
...
FROM test_plans tp
LEFT JOIN release_test_plans rtp ON tp.id = rtp.plan_id
LEFT JOIN releases r ON rtp.release_id = r.id
WHERE tp.project_id = $1
ORDER BY tp.created_at ASC, r.created_at ASC;`,
    [projectId],
  );
  return result.rows;
}
...
export async function AddPlanToRelease(planId: number, releaseId: number) {
  await pool.query(
    `INSERT INTO release_test_plans (release_id, plan_id)
     VALUES ($1, $2)
...
export async function RemovePlanFromRelease(planId: number, releaseId: number) {
  await pool.query(
    `DELETE FROM release_test_plans
     WHERE plan_id = $1 AND release_id = $2`,
    [planId, releaseId],
  );
}
