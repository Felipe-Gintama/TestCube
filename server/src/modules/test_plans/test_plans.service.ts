...
export async function createTestPlan(releaseId: number, name: string, description: string, projectId: number, createdBy?: number | null) {
...
export async function cloneTestPlan(planId: number, newReleaseId: number) {
  ...
  try {
    ...
    const planRes = await client.query(`SELECT * FROM test_plans WHERE id = $1`, [planId]);
    ...
    const insertPlan = await client.query(
      `INSERT INTO test_plans (name, description, project_id, created_by)
       VALUES ($1, $2, $3, $4)
       RETURNING *`,
      [plan.name + " (Copy)", plan.description, plan.project_id, plan.created_by]
    );
    ...
    const testCases = await client.query(`SELECT test_case_id FROM test_plan_cases WHERE test_plan_id = $1`, [planId]);
...
export async function deleteTestPlan(planId: number) {
  ...
  try {
    ...
    await client.query(
      `DELETE FROM release_test_plans WHERE plan_id = $1`,
      [planId]
    );

    await client.query(
      `DELETE FROM test_plan_cases WHERE test_plan_id = $1`,
      [planId]
    );
